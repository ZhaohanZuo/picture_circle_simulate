...
int main() {
	try {
		...
		cols = imgu.cols;
		Scalar car = mean(imgu);
		Mat carvanu(rows, cols, CV_8UC4, car);
...
//#pragma omp parallel for
			for (int index = 0; index < cir.size(); ++index) {
				for (int step = 0; step < climbCount; ++step) {
					tempcir = climb(cir[index], carvan);
...
//#pragma omp parallel for
		for (int i = 0; i < cirnum; i++) {
			...
			cout << "  Best found: (x=" << best.x << ", y=" << best.y << ", r=" << best.rad << ", err=" << best.err << ")" << endl;
